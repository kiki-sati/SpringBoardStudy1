package com.study.ansboard.commons.util;
...
public class StringUtil {
	...
	public static String nullString(String src, String alter) throws Exception {

		if (src == null || src.equals("null") || "".equals(src) || " ".equals(src)) {
			return checkHtmlView(alter).trim();
		} else {
			src = checkHtmlView(src);
			return src.trim();
		}
...
/**
	...
	public static String cutText(String text, final int length, final String suffix) {
	...
	public static Map<String, String> getMenuCd(final String url) throws Exception {
		...
		String regExpPttrn1 = "/([a-zA-Z]+)/([a-zA-Z]+)/([a-zA-Z]+)/([a-zA-Z0-9]+\\.?)";
	...
	public static Map<String, String> getMicroMenuCd(final String url) throws Exception {
		...
		if(matcher.find()) {
			...
			infoMap.put("depth1", matcher2.group(2)); //1차
			infoMap.put("depth2", matcher2.group(3)); //1차
		}
		return infoMap;
	}
	...
	public static String getRandomNumStringSpecialChar(int num) {
		String randomNum = "";
	...
	public static String encodeFileName(String filename, String fileExtnnNm, HttpServletRequest request) throws Exception {
		...
		try {
			fileName = URLEncoder.encode(fileName,"UTF-8") + "."+fileExtnnNm;
		} catch (UnsupportedEncodingException e) {
			log.info("UnsupportedEncodingException");
		}
		
		String resultName = new String( fileName.getBytes("UTF-8"), "ISO-8859-1");
		try{
			// Explorer
			...
			// Safari
				...
				resultName = new String( fileName.getBytes("MS949"),"ISO8859_1");
			}
			// FireFox
	...
	public static String encodingUrl(String url) throws UnsupportedEncodingException {
		String returnUrl = url;
		
		returnUrl = returnUrl.replaceAll("&#35;", "#");
		returnUrl = returnUrl.replaceAll("&#10;", "%20"); // space
		returnUrl = returnUrl.replaceAll("&#33;", "%21"); // !
		returnUrl = returnUrl.replaceAll("&#34;", "%22"); // "
		returnUrl = returnUrl.replaceAll("&#35;", "%23"); // #
		returnUrl = returnUrl.replaceAll("&#36;", "%24"); // $
		returnUrl = returnUrl.replaceAll("&#37;", "%25"); // %
		returnUrl = returnUrl.replaceAll("&#38;", "%26"); // &
		returnUrl = returnUrl.replaceAll("&#39;", "%27"); // '
		returnUrl = returnUrl.replaceAll("&#40;", "%28"); // (
		returnUrl = returnUrl.replaceAll("&#41;", "%29"); // )
		returnUrl = returnUrl.replaceAll("&#42;", "%2A"); // *
		returnUrl = returnUrl.replaceAll("&#43;", "%2B"); // +
		returnUrl = returnUrl.replaceAll("&#44;", "%2C"); // ,
		returnUrl = returnUrl.replaceAll("&#45;", "%2D"); // -
		returnUrl = returnUrl.replaceAll("&#46;", "%2E"); // .
		returnUrl = returnUrl.replaceAll("&#47;", "%2F"); // /
		returnUrl = returnUrl.replaceAll("&#58;", "%3A"); // :
		returnUrl = returnUrl.replaceAll("&#59;", "%3B"); // ;
		returnUrl = returnUrl.replaceAll("&#60;", "%3C"); // <
		returnUrl = returnUrl.replaceAll("&#61;", "%3D"); // =
		returnUrl = returnUrl.replaceAll("&#62;", "%3E"); // >
		returnUrl = returnUrl.replaceAll("&#63;", "%3F"); // ?
		returnUrl = returnUrl.replaceAll("&#64;", "%40"); // @
		returnUrl = returnUrl.replaceAll("&#91;", "%5B"); // [
		returnUrl = returnUrl.replaceAll("&#92;", "%5C"); // \
		returnUrl = returnUrl.replaceAll("&#93;", "%5D"); // ]
		returnUrl = returnUrl.replaceAll("&#94;", "%5E"); // ^
		returnUrl = returnUrl.replaceAll("&#95;", "%5F"); // _
		returnUrl = returnUrl.replaceAll("&#96;", "%60"); // `
		returnUrl = returnUrl.replaceAll("&#123;", "%7B"); // {
		returnUrl = returnUrl.replaceAll("&#124;", "%7C"); // |
		returnUrl = returnUrl.replaceAll("&#125;", "%7D"); // }
		returnUrl = returnUrl.replaceAll("&#126;", "%7E"); // ~
		
		String[] korParams = returnUrl.replaceAll("[^ㄱ-ㅎㅏ-ㅣ가-힣]", "").split("");
		for (String string : korParams) {
			returnUrl = returnUrl.replace(string, URLEncoder.encode(string, "UTF-8"));
		}
	...
	public static String getClassName() {
		...
		String[] eachPath = classPath.split("\\.");
		String className = eachPath[eachPath.length - 1]; 
